...
export function selectWinner(state: ContestState, winnerId: number): ContestState {
  ...
  if (!currentMatch || state.isComplete) return state
  if (winnerId !== currentMatch.leftImage.id && winnerId !== currentMatch.rightImage.id) return state
  const loserInCurrentMatch = currentMatch.leftImage.id === winnerId ? currentMatch.rightImage : currentMatch.leftImage
