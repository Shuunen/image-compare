...
function DragOverlay({ isDraggingLeft, isDraggingOver, nbDraggedFiles }: { isDraggingLeft: boolean; isDraggingOver: boolean; nbDraggedFiles: number }) {
  return (
    <motion.div
      ...
      {nbDraggedFiles === 1 && (
        <motion.div animate={{ scale: isDraggingOver ? 1 : 0.9 }} className="flex w-full justify-around" initial={{ scale: 0.9 }} transition={{ duration: 0.2 }}>
          <motion.div
            ...
            className={cn('rounded-lg bg-background/80 px-6 py-3 text-lg font-semibold text-foreground', isDraggingLeft && 'bg-secondary/90')}
            initial={{ scale: 0.9 }}
            transition={{ duration: 0.2 }}
          >
            Change left image
...
// oxlint-disable-next-line max-lines-per-function, react/no-multi-comp
export function ImageViewer({
  ...
  return (
    ...
    >
      ...
      <DragOverlay isDraggingLeft={isDraggingLeft} isDraggingOver={isDraggingOver} nbDraggedFiles={nbDraggedFiles} />
